...
async function loadComponents() {
    ...
    try {
        ...
        if (sidebarTitle) sidebarTitle.textContent = 'Quiz 2 Menü';

        // DİKKAT: add-user çıkartılıp select-players eklendi
        const modals = [
...
function initQuizSystem() {
    ...
    // YENİ EKLENEN: Backend gelene kadar kayıtlı oyuncu simülasyonu
    ...
    let currentEditingTeamIndex = null; // Hangi takıma oyuncu ekliyoruz takip etmek için
    ...
    document.addEventListener('click', async (e) => {
        ...
        if (e.target.closest('#openShowTeamsBtn')) {
            e.preventDefault();
            window.location.href = 'takimlar.html'; // YÖNLENDİRME İSMİ BURADA GÜNCELLENDİ
        }
        ...
        if (e.target.closest('#openAddQuestionBtn')) {
            ...
            // Quiz 2'de puan girilmesi gerekmez — alanı gizle ve zorunluluğu kaldır
            const pointsInput = document.querySelector('input[name="questionPoints"]');
            if (pointsInput) {
                pointsInput.closest('.input-group').style.display = 'none';
                pointsInput.removeAttribute('required');
                pointsInput.value = '0'; // Backend zorunlu tutuyorsa 0 gönder
            }
        ...
        if (e.target.classList.contains('status-btn')) {
            ...
            try {
                ...
                const response = await fetch(`${BASE_URL}/api/game-room/${ROOM_ID}/state`, {
                    ...
                    body: JSON.stringify({ state: "QUIZ2" })
                });
                ...
                alert('Arka planda odanın durumu başarıyla QUIZ2 olarak güncellendi!');
            } catch (error) {
        ...
        // YENİ EKLENEN: Takım kartı içindeki "Oyuncu Ekle" Butonuna Tıklanınca
        if (e.target.classList.contains('add-player-btn')) {
            ...
            // Havuzdaki tüm oyuncuları listele
            availablePlayersPool.forEach(player => {
                let assignedTeam = null;
                // Oyuncu herhangi bir takımda var mı kontrolü
        ...
        // YENİ EKLENEN: Modal İçindeki "Seçili Oyuncuları Ekle" Butonuna Tıklanınca
        ...
        if (e.target.id === 'addQuestionForm') {
            ...
            try {
                ...
                const payload = {
                    ...
                    points: 0 // Quiz 2'de puan takımlar tarafından belirlenir
                };
    ...
    // YENİ EKLENEN: Takım Kartına Oyuncu Ekle Butonu Konuldu
    ...
    // Takımları Gör modalı için — /api/team/all
